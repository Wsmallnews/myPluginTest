...
export default {
  ...
  methods: {
    handleSubmit(name) {
      ...
      _this.$refs[name].validate((valid) => {
        if (valid) {
          var method = 'post'
          var url = '/adminapi/salesmans'
          if (_this.formValidate.id) { // 编辑
            method = 'patch'
            var url = '/adminapi/salesmans/' + _this.formValidate.id
          }
          ...
          Util.ajax({
            ...
            success: function(result) {
              if (result.error == 0) {
                ...
                _this.$router.push('/salesmanManage/salesmans/index')
              } else {
                _this.$Notice.error({
                  title: '提示',
                  desc: result.info
  ...
  created() {
    ...
    if (_this.$route.params.id != undefined) {
      _this.type = 'edit'
      Util.ajax({
        url: '/adminapi/salesmans/' + _this.$route.params.id,
        method: 'get',
        success: function(result) {
          if (result.error == 0) {
