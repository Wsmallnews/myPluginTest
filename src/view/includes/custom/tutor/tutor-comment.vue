<template lang="html">
  <div class="tutor-comments-index">
    <myTable ref="listTable" :listConf="listConf" @select="selectRow" @searchReset="searchReset" :noSearch="noSearch">
      <template slot="formItem" >
        <!-- <Form-item prop="user_id">
          <Select style="width:200px" v-model="listConf.searchParams.user_id" clearable placeholder="导师">
            <Option v-for="(tutor, index) in tutors" :key="index" :label="tutor.name" :value="tutor.id" ></Option>
          </Select>
        </Form-item> -->
...
export default {
  ...
  methods: {
    ...
    qulity (id, is_quality) {
      var _this = this
      Util.ajax({
        url: '/adminapi/tutorComments/' + id + '/setQuality',
        method: 'patch',
    ...
    getTutors() {
      ...
      var _this = this;
      Util.ajax({
        url: '/adminapi/tutors/all',
        method: 'get',
        success: function(result) {
          if (result.error == 0) {

            _this.tutors = result.result;
          } else {
            _this.$Notice.error({
              title: '提示',
              desc: result.info
    ...
    showReply (item) {
      ...
      // this.replyForm.user_id = item.user_id;
    ...
    ok: function () {
      ...
      _this.$refs['replyForm'].validate((valid) => {
        if (valid) {
          ...
          Util.ajax({
            url: '/adminapi/tutorComments/'+ _this.replyForm.id +'/reply',
            method: 'post',
            data: {user_id: this.replyForm.user_id, content: this.replyForm.content},
            success: function(result) {
    ...
    statusOper: function () {
      ...
      _this.$refs['statusForm'].validate((valid) => {
        if (valid) {
          ...
          Util.ajax({
            url: '/adminapi/tutorComments/'+ _this.statusForm.id +'/statusOper',
            method: 'patch',
