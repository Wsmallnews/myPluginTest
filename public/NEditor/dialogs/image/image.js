    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadSuccess', function (file, res) {
                var data = {
                    code: res.error === 0 ? 200 : 0,
                    state: res.error === 0 ? 'SUCCESS' : 'FAIL',
                    original: res.data ? res.data.origin_url : '',
                    size: file.size,
                    title: file.name,
                    type: file.type,
                ...
                var $file = $('#' + file.id);
                try {
                    if (editor.getOpt("imageUploadService")(_this, editor).getResponseSuccess(data)) {
                        _this.imageList.push(data);
                        $file.append('<span class="success"></span>');
                    } else {
                        $file.find('.error').text(data.message).show();
                    }
                } catch (e) {
